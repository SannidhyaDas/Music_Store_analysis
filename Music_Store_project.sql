...
SELECT COUNT(*) AS  Total_Invoices_by_country , billing_country
FROM invoice 
GROUP BY billing_country
ORDER BY Total_Invoices_by_country DESC;
...
-- one city has the highest sum of invoice totals.Return both the city name 
...
SELECT email,first_name,last_name, genre.name AS Name 
FROM customer 
JOIN invoice ON customer.customer_id=invoice.customer_id 
...
--(here we use the concept of common table expressions )
...
--(here we use the concept of common table expressions and ROW_NUMBER )

WITH popular_genre_byCountry AS (
 ...
 SELECT COUNT(invoice_line.quantity) AS purchases, customer.country, 
  ...
  SELECT * FROM popular_genre_byCountry WHERE RowNo <= 1
